...
export const LoginPage: React.FC = () => {
  ...
  useEffect(() => {
    if (!googleClientId || selectedRole !== 'PATIENT') return;
    ...
    const setupGSI = () => {
      ...
      if (google?.accounts?.id) {
        try {
          google.accounts.id.initialize({
            ...
            callback: async (response: { credential?: string }) => {
              if (response?.credential) {
                setFormError(null);
                setSuccessMessage('Connexion Google en cours...');
                const res = await loginWithGoogleIdToken(response.credential, selectedRole);
                if (res.success) {
                  setSuccessMessage('Connexion réussie ! Redirection...');
                  setTimeout(() => redirectAfterAuth(selectedRole), 400);
                } else {
                  setFormError(res.error || 'Erreur lors de la validation du compte Google.');
                }
              }
            },
            auto_select: false,
          ...
          const container = document.getElementById('google-gsi-render');
          if (container) {
  ...
  }, [googleClientId, selectedRole, loginWithGoogleIdToken]);

  const handleGoogleLogin = async () => {
    setFormError(null);
    setSuccessMessage(null);

    // Si Google Identity Services est configuré, déclencher le prompt natif Google
    const google = (window as any).google;
    if (googleClientId && google?.accounts?.id) {
      google.accounts.id.prompt((notification: any) => {
        if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
          // Si le prompt one-tap est ignoré ou déjà fermé, repli sur OAuth classique
