...
export const DEFAULT_EMAIL_DATA: EmailData = {
  ...
  loginUrl: 'https://clinigo.fr/connexion',
  contactUrl: 'https://clinigo.fr/#contact'
};

/**
 * Generate 100% compliant, responsive HTML for email clients (Resend, Gmail, Outlook, Apple Mail)
 */
export function generateWelcomeEmailHtml(data: EmailData = DEFAULT_EMAIL_DATA): string {
  const firstName = data.firstName || 'Dimitri';
  const loginUrl = data.loginUrl || 'https://clinigo.fr';
  const contactUrl = data.contactUrl || 'https://clinigo.fr';
...
interface WelcomeEmailMockupProps {
  data: EmailData;
  isMobile?: boolean;
}

export const WelcomeEmailMockup: React.FC<WelcomeEmailMockupProps> = ({
  data,
  isMobile = false
}) => {
  const firstName = data.firstName || 'Dimitri';
  ...
  return (
    <div
      className={`mx-auto bg-white rounded-2xl md:rounded-3xl border border-[#E5E7EB] transition-all duration-300 ${
        isMobile
          ? 'w-full max-w-[390px] shadow-sm'
          : 'w-full max-w-[600px] shadow-lg shadow-black/[0.03]'
      }`}
      style={{
        fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif"
      }}
    >
      {/* HEADER : LOGO CLINIGO CENTRÉ */}
      <div className="pt-9 pb-6 px-6 sm:px-10 text-center border-b border-[#F3F4F6]">
        <img
          src="/assets/clinigo-logo.png"
          alt="Clinigo"
          className="h-8 sm:h-9 mx-auto object-contain"
          onError={(e) => {
            // Fallback text if image not reachable
            const target = e.target as HTMLElement;
            target.style.display = 'none';
            if (target.parentElement) {
              target.parentElement.innerHTML = '<span style="font-size: 22px; font-weight: 800; letter-spacing: -0.03em; color: #111827;">CLINI<span style="color: #2563EB;">GO</span></span>';
            }
          }}
        />
      </div>
      ...
      {/* BODY CONTENT */}
      <div className={`px-6 sm:px-11 ${isMobile ? 'py-7' : 'py-9'}`}>
        

        {/* TITRE ET MESSAGE D'ACCUEIL */}
        <div className="text-center mb-7">
          <h1 className="text-2xl sm:text-[26px] font-extrabold text-[#111827] tracking-tight mb-2">
            Bonjour {firstName} 👋
          </h1>
          <p className="text-base sm:text-lg font-semibold text-[#2563EB] mb-3">
            Bienvenue sur Clinigo.
          </p>
          <p className="text-sm sm:text-[15px] leading-relaxed text-[#4B5563] max-w-[480px] mx-auto">
            Votre compte a bien été créé.<br />
            Vous pouvez maintenant utiliser Clinigo pour organiser vos transports sanitaires simplement et en toute tranquillité.
          </p>
        </div>
        ...
        <div className="text-center mb-8">
          <a
            href="https://clinigo.fr"
            onClick={(e) => e.preventDefault()}
            className="inline-flex items-center justify-center px-8 py-3.5 sm:py-4 rounded-xl bg-[#2563EB] hover:bg-[#1D4ED8] text-white font-bold text-xs sm:text-sm tracking-wider uppercase shadow-md shadow-[#2563EB]/20 transition-all hover:scale-[1.01] active:scale-[0.99] cursor-pointer"
          >
            ACCÉDER À MON COMPTE
          </a>
        </div>

        {/* CARTE INFORMATIVE LÉGÈREMENT BLEUTÉE */}
        <div className="p-4 sm:p-5 rounded-2xl bg-[#F0F7FF] border border-[#DBEAFE] mb-7">
          <div className="flex items-start gap-3">
            <div className="text-xl shrink-0 mt-0.5">🚑</div>
            <div>
              <h4 className="text-sm font-bold text-[#1E40AF] mb-1">
                Votre espace Clinigo
              </h4>
              <p className="text-xs sm:text-[13px] leading-relaxed text-[#3B82F6]">
                Depuis votre espace personnel, vous pourrez demander un transport, consulter vos réservations et suivre vos courses en temps réel.
              </p>
            </div>
          </div>
        ...
        {/* 3 FONCTIONNALITÉS (Horizontal Desktop, Vertical Mobile) */}
        <div className={`grid gap-3 mb-8 ${isMobile ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-3'}`}>
          {/* Bloc 1 */}
          <div className="p-4 rounded-xl bg-[#FAFAFA] border border-[#F3F4F6] text-left">
            <div className="text-xl mb-2">📅</div>
            <h5 className="text-xs sm:text-sm font-bold text-[#111827] mb-1">
              Réserver
            </h5>
            <p className="text-[11px] sm:text-xs leading-relaxed text-[#6B7280]">
              Organisez votre transport sanitaire simplement.
            </p>
          </div>
        ...
        {/* SÉPARATION LÉGÈRE */}
        <hr className="border-0 border-t border-[#E5E7EB] my-6" />

        {/* SECTION SUPPORT */}
        <div className="text-center">
          <h5 className="text-sm font-bold text-[#111827] mb-1">
            Besoin d'aide ?
          </h5>
          <p className="text-xs sm:text-[13px] text-[#6B7280] mb-2">
            Notre équipe Clinigo est disponible pour répondre à vos questions.
          </p>
          <a
            href="https://clinigo.fr/#contact"
            onClick={(e) => e.preventDefault()}
            className="inline-flex items-center gap-1 text-xs sm:text-sm font-bold text-[#2563EB] hover:text-[#1D4ED8] hover:underline"
          >
            <span>Contacter Clinigo</span>
            <span>&rarr;</span>
          </a>
        </div>
      ...
      {/* FOOTER SOBRE */}
      <div className="bg-[#FAFAFA] border-t border-[#F3F4F6] py-7 px-6 text-center rounded-b-2xl md:rounded-b-3xl">
        <div className="mb-2.5">
          <img
            src="/assets/clinigo-logo.png"
            alt="Clinigo"
            className="h-5 mx-auto object-contain opacity-70 grayscale hover:grayscale-0 transition-all"
          />
        </div>
        ...
        <p className="text-[11px] text-[#9CA3AF] mb-2">
          <a href="https://clinigo.fr" className="hover:text-[#6B7280]">Clinigo.fr</a>
          {' · '}
          <a href="https://clinigo.fr/#contact" className="hover:text-[#6B7280]">Contact</a>
          {' · '}
          <a href="https://clinigo.fr/confidentialite" className="hover:text-[#6B7280]">Confidentialité</a>
        </p>
